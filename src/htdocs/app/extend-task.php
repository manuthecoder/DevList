...
<?php
    try {
    $dbh = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
    $sql = "SELECT * FROM todo WHERE project=" . $_GET['id']. " AND stat='Done' ORDER by id DESC LIMIT ".$_GET['count'];
    $users = $dbh->query($sql);
    $users = $dbh->query($sql);
    $row_count = $users->rowCount();
    if($row_count !== 0) {
    ...
    echo '<button onclick="AJAX_LOAD(\'extend-task.php?id='.$_GET['id'].'&count='.$count.'\')" class="btn darken-3 red waves-effect waves-light" style="width: 100%;display:block;">Load More</button>';
    }
    else {
